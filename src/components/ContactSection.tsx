      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-base leading-8 text-brand-ink/70">
            اتصل بنا مباشرة أو أرسل طلبك وسيفتح لك واتساب برسالة جاهزة تحتوي على تفاصيل طلبك.
          </p>
        </Reveal>
            ...
            <div className="rounded-[1.75rem] border border-beige/70 bg-cream p-7 shadow-soft md:p-9">
              {status === "success" ? (
                <div className="flex min-h-[360px] flex-col items-center justify-center text-center" role="status">
                  ...
                  <h3 className="mt-5 text-2xl font-bold text-brand">تم تجهيز طلبك</h3>
                  <p className="mt-3 max-w-sm leading-8 text-brand-ink/70">
                    تم فتح واتساب برسالة جاهزة تحتوي على بيانات طلبك. إذا لم يفتح تلقائيًا، يمكنك التواصل مباشرة على{" "}
                ...
                <form onSubmit={handleSubmit} className="grid gap-5 sm:grid-cols-2">
                  <div>
                    <label htmlFor="name" className="mb-1.5 block text-sm font-bold text-brand">الاسم</label>
                    <input id="name" name="name" type="text" required minLength={2} autoComplete="name" placeholder="اسمك الكريم" className={inputCls} />
                  </div>
                  <div>
                    <label htmlFor="phone" className="mb-1.5 block text-sm font-bold text-brand">رقم الجوال</label>
                    <input id="phone" name="phone" type="tel" required inputMode="tel" autoComplete="tel" dir="ltr" placeholder="05xxxxxxxx" className={`${inputCls} text-right placeholder:text-left`} />
                  </div>
                  <div className="sm:col-span-2">
                    <label htmlFor="service" className="mb-1.5 block text-sm font-bold text-brand">الخدمة المطلوبة</label>
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <textarea id="message" name="message" rows={4} placeholder="اكتب أي تفاصيل إضافية عن طلبك..." className={`${inputCls} resize-y`} />
                  </div>
                  {status === "error" && <p role="alert" className="sm:col-span-2 rounded-xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{errorMsg}</p>}
                  <div className="sm:col-span-2">
                    <button type="submit" disabled={status === "loading"} className="w-full rounded-full bg-brand px-8 py-3.5 text-base font-bold text-cream shadow-card transition-all hover:-translate-y-0.5 hover:bg-brand-deep disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto">
                      {status === "loading" ? "جارٍ فتح واتساب..." : "إرسال الطلب عبر واتساب"}
                    </button>
                  </div>
                </form>
              )}
